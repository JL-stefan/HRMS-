...
def get_test_data(path, sheetname):
    ...
    if nrows > 1:
        for i in range(1, nrows):
            ...
            user['date'] = sheet.cell_value(i, 2)
            user['reason'] = sheet.cell_value(i, 3)
            user['processer1_name']= sheet.cell_value(i, 4)
            user['processer1_username'] = sheet.cell_value(i, 5)
            user['processer2_name'] = sheet.cell_value(i, 6)
            user['processer2_username'] = sheet.cell_value(i, 7)
            user['processer3_name'] = sheet.cell_value(i, 8)
            user['processer3_username'] = sheet.cell_value(i, 9)
            user['processer4_name'] = sheet.cell_value(i, 10)
            user['processer4_username'] = sheet.cell_value(i, 11)
            user['processer5_name'] = sheet.cell_value(i, 12)
            user['processer5_username'] = sheet.cell_value(i, 13)
            user['processer6_name'] = sheet.cell_value(i, 14)
            data.append(user)
        return data
    else:
        return 0
...
class TestSign(unittest.TestCase):
    ...
    def test_sign(self,user):
        ...
        # 开始申请签卡
        apply_test.apply_sign(date, reason)
        # 选择下一个审批人
...
#
    # test_dir = r'C:\Users\Administrator\PycharmProjects\HRMS'
